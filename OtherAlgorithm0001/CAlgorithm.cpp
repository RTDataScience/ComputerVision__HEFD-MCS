...
CAlgorithm::CAlgorithm()
{
    ...
    m_pucRedMeanAndGBFilterImage = NULL;

    m_pucTransLittleRGB = NULL;
...
CAlgorithm::~CAlgorithm()
{
    ...
    SAFE_DELETE_ARRAY(m_pucRedMeanAndGBFilterImage);

    SAFE_DELETE_ARRAY(m_pucTransLittleRGB);
...
)
{
    ...
    if (m_pucRedMeanAndGBFilterImage == NULL)
    {
        m_pucRedMeanAndGBFilterImage = new unsigned char[m_lReduceROILength];
        memset(m_pucRedMeanAndGBFilterImage, 0x00, m_lReduceROILength);
    }

    if (m_pucTransLittleRGB == NULL)
    {
        m_pucTransLittleRGB = new unsigned char[m_lReduceROILength];
        memset(m_pucTransLittleRGB, 0x00, m_lReduceROILength);
    }
...
AlgProcessing_BitDepth4()
{
    ...
    if (RedMeanAndGBFilterImage(
        m_pucReduceROI,
        m_lReduceROIWidth,
        m_lReduceROIHeight,
        m_lOriginBitDepth,
        iRedMean,
        m_pucRedMeanAndGBFilterImage
    ) == false)
    {
        return false;
    }
    
    if (TransLittleRGBImage(
        m_pucRedMeanAndGBFilterImage,
        m_lReduceROIWidth,
        m_lReduceROIHeight,
        m_lOriginBitDepth,
    ...
    //        if (
    //            m_pucTransLittleRGB[iIndex + 2] > 0 ||
    //            m_pucTransLittleRGB[iIndex + 1] > 0 ||
    //            m_pucTransLittleRGB[iIndex + 0] > 0
    //            )
    //        {
                /*************************************************************
...
)
{
    ...
    for (iY = 0; iY < iHeight; iY++)
    {
        for (iX = 0; iX < iWidth; iX++)
        {
            ...
            {
            case 4:
                dbRedSum = dbRedSum + pInImage[lSourceIndex + 2];
                break;
            case 3:
                dbRedSum = dbRedSum + pInImage[lSourceIndex + 2];
                break;
    ...
    *iOutRedMean = (int)(dbRedSum / iCount);
    return true;
}


bool CAlgorithm::
RedMeanAndGBFilterImage(
    unsigned char* pInImage,
    int iInWidth,
    int iInHeight,
    int iInBitDepth,
...
)
{
    ...
    for (iY = 0; iY < iHeight; iY++)
    {
        for (iX = 0; iX < iWidth; iX++)
        {
            ...
            iRGB_G = pInImage[lSourceIndex + 1];
            iRGB_B = pInImage[lSourceIndex + 0];
            iRGB_Sum = iRGB_R + iRGB_G + iRGB_B;
            fRGB_r = ((float)iRGB_R) / ((float)iRGB_Sum);
            fRGB_g = ((float)iRGB_G) / ((float)iRGB_Sum);
            fRGB_b = ((float)iRGB_B) / ((float)iRGB_Sum);
            if (
                (fRGB_r >= ((1.1403f * fRGB_g) - 0.0759f)) &&
                (fRGB_r <= ((-0.9889f * fRGB_g) + 0.9913f)) &&
                (fRGB_r >= ((-2.0771f * fRGB_g) + 1.0251f)) &&
                (fRGB_r <= ((-1.8813f * fRGB_b) + 0.95824f)) &&
                (fRGB_r <= ((95.3478f * fRGB_b) + 0.1707f)) &&
                (fRGB_r >= ((-0.5427f * fRGB_b) + 0.5063f)) &&
                (fRGB_g >= ((0.8459f * fRGB_b) + 0.0482f)) &&
                (fRGB_g <= ((-0.4608f * fRGB_b) + 0.4954f)) &&
                (fRGB_g <= ((191.9565f * fRGB_b) - 0.6207f))
                )
            {
                switch (iBitDepth)
                {
