...
void CALLBACK fniSAPNVRSDKControl_ImageCallback ( unsigned long ulUserParam, unsigned char* pszBuf, unsigned long ulLen )
{
    if ( ulUserParam == NULL || pszBuf == NULL || ulLen <= 0 )
    {
        return;
    }
    BITMAPINFOHEADER* pHeader = (BITMAPINFOHEADER*) pszBuf;
    DWORD dwWidth = pHeader->biWidth;
    DWORD dwHeight = abs ( pHeader->biHeight );
    unsigned char* buf = pszBuf;
    unsigned long len = ulLen;
    if ( dwWidth <= 0 || dwHeight <= 0 || dwWidth >= 2560 || dwHeight >= 2048 )
    {
        return;
    }
    CHEFD_MCSView* pView = (CHEFD_MCSView*) (__int64) ulUserParam; 
        ...
        )
    {
        ...
        if (pView->stCHEFD_MCSOutput.bOutTrigger01 == true)
        {
    ...
    else if ( pView->iChoiceMode == 2 )
    {
        ...
        double dbOtherAlgorithm0001 = 0.0f;
