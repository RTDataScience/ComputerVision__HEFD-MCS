...
CAlgorithm::CAlgorithm()
{
    ...
    m_pucFireRegionIsolationImage = NULL;
...
CAlgorithm::~CAlgorithm()
{
    ...
    SAFE_DELETE_ARRAY(m_pucFireRegionIsolationImage);
...
)
{
    ...
    if (m_pucFireRegionIsolationImage == NULL)
    {
        m_pucFireRegionIsolationImage = new unsigned char[m_lReduceROILength];
        memset(m_pucFireRegionIsolationImage, 0x00, m_lReduceROILength);
    }
...
AlgProcessing_BitDepth4()
{
    ...
    int iInRedThreshold = 125;
    // ST is the saturation channel threshold (range: 55 -> 65)
    int iInSaturationThreshold = 60;
    ...
    if ( FireRegionIsolationImage(
        ...
        m_lOriginBitDepth,
        iInRedThreshold,
        iInSaturationThreshold,
        m_pucFireRegionIsolationImage
    ) == false)
    {
        return false;
    }
    ...
    //        if (
    //            m_pucFireRegionIsolationImage[iIndex + 2] > 0 ||
    //            m_pucFireRegionIsolationImage[iIndex + 1] > 0 ||
    //            m_pucFireRegionIsolationImage[iIndex + 0] > 0
    //            )
    //        {
                /*************************************************************
            ...
            case 4:
                ...
                if (RGBIn.r > iInRedThreshold)
                {
                        ...
                        )
                    {
                        ...
                        {
                            pOutImage[lSourceIndex + 3] = pInImage[lSourceIndex + 3];
